...
import { saveContractSignature } from './contract-api';
...
export function ContractSignatureDialog({ open, contractId, signer, onClose, onSigned }: Props) {
  ...
  const save = async () => {
    ...
    try {
      const signatureUrl = await saveContractSignature(
        contractId,
        signer,
  ...
  return (
    <Dialog fullWidth maxWidth="sm" open={open} onClose={onClose}>
      <DialogTitle>{signer === 'issuer' ? 'ลงชื่อผู้รับจ้าง' : 'ลงชื่อผู้ว่าจ้าง'}</DialogTitle>
      <DialogContent>
        <Box
          component="canvas"
          ref={canvasRef}
